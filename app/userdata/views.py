...
def differenttradingpartners_vendor(userid, otherid):
    ...
    getuserlocation = userimagelocation(userid=userid)
    # get stats if vendor
    itemsbought = db.session.query(StatisticsVendor).filter(userid == StatisticsVendor.vendorid).first()
    # find path of the user
...
def affstats(userid, amount, currency):
    ...
    aff_stats = db.session.query(AffiliateStats).filter(userid == AffiliateStats.userid).first()
